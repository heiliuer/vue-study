<template>
  <div class="home">
    <div class="house-group_wrap">
      <transition-group name="list" class="house-group" tag="div">
        <HouseGroupItem
...
export default class Home extends Vue {
  ...
  private initList(): HouseItem[] {
    let refIndex = 1;
    const list: HouseItem[] = Array.apply(0, new Array(200)).map(
      (value, index) => {
        const length = Math.ceil(Math.random() * 3);
        refIndex += length;
        const items: HouseItemItem[] = Array.apply(0, new Array(length)).map(
          (value2, index2) => {
